...
const Hero = () => {
  const [formState, setFormState] = useState('button'); // 'button', 'form', 'submitted'
  const [placeholder, setPlaceholder] = useState('');
  
  useEffect(() => {
    let timeout;
    if (formState === 'form') {
      const text = "Enter Your Email Here For Early Access";
      let currentIndex = 0;
      setPlaceholder('');
      ...
      const text = "You Will Receive Notifications By Email";
      let currentIndex = 0;
      setPlaceholder('');
      ...
      const typeText = () => {
        if (currentIndex < text.length) {
          ...
          setTimeout(() => setFormState('button'), 4000);
        }
      };
      typeText();
    }
